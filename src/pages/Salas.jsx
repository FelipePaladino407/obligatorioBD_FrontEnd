...
export default function Salas() {
  ...
  // nuevo: handler para eliminar sala
  const handleDeleteSala = async (sala) => {
    setDeleteError(null);
    const ok = window.confirm(
      `¿Está seguro que desea eliminar la sala "${sala.nombre_sala}" del edificio "${sala.edificio}"?`
    );
    if (!ok) return;

    try {
      await deleteSala(sala.nombre_sala, sala.edificio, token);
      await loadSalas();
    } catch (e) {
      setDeleteError(e.message || "Error al eliminar sala");
    }
  };
  ...
  return (
    <div className="salas-container">
      <div className="salas-wrapper">
        ...
        {/* nuevo: mostrar error al eliminar */}
        {deleteError && (
              ...
              <div className="salas-grid">
                {salas.map((s) => (
                  ...
                  >
                    ...
                    {user?.is_admin && (
                      <div className="sala-admin-actions">
                        <button
